		...
		>
			...
			<b-form-row>
				<b-form-group label="Konten" class="col">
                    ...
                    <!-- Fallback -->
                    <b-textarea v-model="saveData.konten" style="height: 200px" v-if="isIE10Mode" placeholder="Syarat dan Ketentuan" />
				</b-form-group>
			</b-form-row>
...
export default {
    ...
    methods:{
        loadData(){
            ...
            axios
				.get(this.publicUrl + "api/config/1")
				.then((response) => { 
					// handle success
        ...
        update(){
            ...
            axios({
                method: "put",
                url: this.publicUrl + "api/config/1",
                data: this.saveData
            })
                .then((response) => {
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
					...
					if (data_rule.rule.find(o => o.name === 'syarat_ketentuan_edit')) {	
						this.hasAccess.edit = data_rule.rule.find(o => o.name === 'syarat_ketentuan_edit').active
					}
				}else{
					this.hasAccess = {
						edit: true,
