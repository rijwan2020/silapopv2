<template>
    <div>
        <div class="d-flex justify-content-between align-items-center w-100 mb-0 border-bottom">
            <div class="d-flex align-items-center">
                <h5 class="m-1 d-inline-block text-nowrap font-weight-normal">
                    Data / Baku Lahan / Rencana & Realisasi Tanam
                </h5>
            </div>
            <div class="m-1">
                <b-btn
                ...
                <b-btn
					variant="primary"
					v-b-tooltip.hover
					title="Tambah rencana dan realisasi tanam"
					@click="showModal('add')"
                    v-if="hasAccess.add"
                    size="sm">
		...
		<div class="row pt-3 text-center">
			<div class="col-md-6">
				<b-card class="mb-4">
					<p class="card-text">Total Rencana Tanam</p>
					<h4 class="card-title">{{ data.total_rencana_tanam }} Ha</h4>
				</b-card>
			</div>
			<div class="col-md-6">
				<b-card class="mb-4">
					<p class="card-text">Total Realisasi Tanam</p>
					<h4 class="card-title">{{ data.total_realisasi_tanam }} Ha</h4>
				</b-card>
			</div>
		</div>
				...
				>
					...
					<template v-slot:cell(actions)="data">
						<b-btn
							...
							v-b-tooltip.hover
							:title="`Edit Rencana dan Realisasi tanam`"
						...
						<b-btn
							...
							v-b-tooltip.hover
							:title="`Hapus Rencana dan Realisasi tanam`"
		...
		<b-modal
			...
			<div slot="modal-title">
				{{ mode == "add" ? "Add" : "Update" }} Baku Lahan
			</div>
            ...
            <b-form-row>
                <b-form-group label="Tanggal" class="d-inline-block w-auto m-1">
                    <b-datepicker 
                        ...
                        :max="currentDate"
                        min="2016-01-01"
                        v-model="saveData.tanggal_input" />
                </b-form-group>
            </b-form-row>
            ...
            <!-- Rencana Tanam -->
			<b-form-row>
				<b-form-group label="Rencana Tanam" class="col">
                    <b-input-group>
                        <b-input
                            placeholder="Luas"
                            v-model="saveData.rencana_tanam"
                        />
            ...
            <!-- Realisasi Tanam -->
			<b-form-row>
				<b-form-group label="Realisasi Tanam" class="col">
                    <b-input-group>
                        <b-input
                            placeholder="Luas"
                            v-model="saveData.realisasi_tanam"
                        />
                        <b-input-group-text slot="append">Ha</b-input-group-text>
                    </b-input-group>
				</b-form-group>
...
export default {
    name: 'list',
	metaInfo: {
		title: 'Rencana dan Realisasi Tanam'
    },
    components: {
        LaddaBtn,
        SweetModal
    ...
    data: () => ({
		...
		fields: [
			...
			{ key: "tanggal_input", label:"Tanggal", sortable: false, tdClass: "align-middle", thClass: "text-nowrap align-middle text-center" },
			...
			{ key: "rencana_tanam", label:"Rencana Tanam [Ha]", sortable: false, tdClass: "align-middle", thClass: "text-nowrap align-middle text-center" },
			{ key: "realisasi_tanam", label:"Realisasi Tanam [Ha]", sortable: false, tdClass: "align-middle", thClass: "text-nowrap align-middle text-center" },
    ...
    methods:{
        ...
        loadData(page = 1){
			...
			axios
				.get(this.publicUrl + "api/bakulahandetail", {
        ...
        download(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/bakulahandetail/download",
                data: params
            }).catch((error) => {
        ...
        cekParam(){
            if (this.$route.params.id) {
                axios
                    .get(this.publicUrl + "api/bakulahan/" + this.$route.params.id)
                    .then((response) => {
                        ...
                        this.pilihTahun = this.saveData.tahun = response.data.data.tahun
                        this.pilihJenis = this.saveData.jenis = response.data.data.jenis
                    })
                    .catch((error) => {
                        this.swr();
                    });
            }else{
                this.notif("warning", "Warning", "Tidak dapat mengakses halaman ini.");
                this.$router.push('bakulahan');
            }
        },
        ...
        showModal(mode, i = null){
            ...
            if (mode == 'edit') {
                ...
                this.saveData = {
                    ...
                    jenis: data.jenis,
                    rencana_tanam: data.rencana_tanam,
                ...
                this.saveData = {
                    tanggal_input: this.currentDate,
        ...
        save() {
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/bakulahandetail",
                data: this.saveData
            })
                .then((response) => {
        ...
        update(){
            ...
            axios({
                method: "put",
                url: this.publicUrl + "api/bakulahandetail/" + this.id,
                data: this.saveData
            })
                .then((response) => {
                    if(this.fileAttachment != null){
        ...
        uploadFile(){
            ...
            axios.post(
                this.publicUrl + "api/bakulahandetail/file",
                formData,
                {
                    headers: {
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
					...
					if (data_rule.rule.find(o => o.name === 'rencana_realisasi_add')) {	
						this.hasAccess.add = data_rule.rule.find(o => o.name === 'rencana_realisasi_add').active
					}
					if (data_rule.rule.find(o => o.name === 'rencana_realisasi_edit')) {	
						this.hasAccess.edit = data_rule.rule.find(o => o.name === 'rencana_realisasi_edit').active
					}
					if (data_rule.rule.find(o => o.name === 'rencana_realisasi_delete')) {	
						this.hasAccess.delete = data_rule.rule.find(o => o.name === 'rencana_realisasi_delete').active
					}
					if (data_rule.rule.find(o => o.name === 'rencana_realisasi_ver_kota')) {	
						this.hasAccess.verkota = data_rule.rule.find(o => o.name === 'rencana_realisasi_ver_kota').active
					}
					if (data_rule.rule.find(o => o.name === 'rencana_realisasi_ver_kec')) {	
						this.hasAccess.verkec = data_rule.rule.find(o => o.name === 'rencana_realisasi_ver_kec').active
					}
				}else{
					this.hasAccess = {
						add: true,
        ...
        verifikasi(i, tingkat, val){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/bakulahandetail/verifikasi/" + data.id,
                data: saveData
            })
                .then((response) => {
        ...
        processVrf(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/bakulahandetail/vrf_all",
                data: params
            })
            .then((response) => {
