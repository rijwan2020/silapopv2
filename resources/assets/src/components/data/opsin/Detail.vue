<template>
    <div>
        <div class="d-flex justify-content-between align-items-center w-100 mb-0 border-bottom">
            <div class="d-flex align-items-center">
                <h5 class="m-1 d-inline-block text-nowrap font-weight-normal">
                    Data / Data Alsin / LTT/LTP & Kerja Alsintan
                </h5>
            </div>
            <div class="m-1">
                <b-btn
                ...
                <b-btn
					variant="primary"
					v-b-tooltip.hover
					title="Tambah LTT/LTP & Kerja Alsintan"
					@click="showModal('add')"
                    v-if="hasAccess.add"
                    size="sm">
		...
		<div class="row pt-3 text-center">
			<div class="col-md-6">
				<b-card class="mb-4">
					<p class="card-text">Jumlah LTT/LTP</p>
					<h4 class="card-title">{{ data.total_ltt_ltp }} Ha</h4>
				</b-card>
			</div>
			<div class="col-md-6">
				<b-card class="mb-4">
					<p class="card-text">Jumlah Alsin</p>
				...
				>
					...
					<template v-slot:cell(actions)="data">
						<b-btn
							...
							v-b-tooltip.hover
							:title="`Edit LTT/LTP & Kerja Alsintan`"
						...
						<b-btn
							...
							v-b-tooltip.hover
							:title="`Hapus LTT/LTP & Kerja Alsintan`"
		...
		<b-modal
			...
			<div slot="modal-title">
				{{ mode == "add" ? "Add" : "Update" }} LTT/LTP & Kerja Alsintan
			</div>
            ...
            <!-- LTT/LTP -->
			<b-form-row>
				<b-form-group label="LTT/LTP" class="col">
                    <b-input-group>
                        <b-input
                            placeholder="LTT/LTP"
                            v-model="saveData.ltt_ltp"
                        />
            ...
            <!-- Alsin -->
			<b-form-row>
				<b-form-group label="Alsin" class="col">
                    <b-input-group>
                        <b-input
                            placeholder="Alsin"
                            v-model="saveData.alsin"
                        />
...
export default {
    ...
    data: () => ({
		...
		fields: [
			...
			{ key: "ltt_ltp", label:"LTT/LTP [Ha]", sortable: false, tdClass: "align-middle", thClass: "text-nowrap align-middle text-center" },
			{ key: "alsin", label:"Alsin [Unit]", sortable: false, tdClass: "align-middle", thClass: "text-nowrap align-middle text-center" },
    ...
    methods:{
        ...
        loadData(page = 1){
			...
			axios
				.get(this.publicUrl + "api/opsindetail", {
        ...
        download(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/opsindetail/download",
                data: params
            }).catch((error) => {
        ...
        cekParam(){
            if (this.$route.params.id) {
                axios
                    .get(this.publicUrl + "api/opsin/" + this.$route.params.id)
                    .then((response) => {
                        ...
                        this.pilihJenis = this.saveData.jenis_id = response.data.data.jenis_id
                    })
                    .catch((error) => {
                        this.swr();
                    });
        ...
        save() {
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/opsindetail",
                data: this.saveData
            })
                .then((response) => {
        ...
        update(){
            ...
            axios({
                method: "put",
                url: this.publicUrl + "api/opsindetail/" + this.id,
                data: this.saveData
            })
                .then((response) => {
                    if(this.fileAttachment != null){
        ...
        uploadFile(){
            ...
            axios.post(
                this.publicUrl + "api/opsindetail/file",
                formData,
                {
                    headers: {
        ...
        validate() {
            ...
            if (this.saveData.ltt_ltp <= 0 && this.saveData.alsin <= 0) {
                this.notif("danger", "Error", "LTT/LTP / Alsin tidak boleh kosong.");
                return false;
            }
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
					...
					if (data_rule.rule.find(o => o.name === 'alsin_detail_add')) {	
						this.hasAccess.add = data_rule.rule.find(o => o.name === 'alsin_detail_add').active
					}
					if (data_rule.rule.find(o => o.name === 'alsin_detail_edit')) {	
						this.hasAccess.edit = data_rule.rule.find(o => o.name === 'alsin_detail_edit').active
					}
					if (data_rule.rule.find(o => o.name === 'alsin_detail_delete')) {	
						this.hasAccess.delete = data_rule.rule.find(o => o.name === 'alsin_detail_delete').active
					}
					if (data_rule.rule.find(o => o.name === 'alsin_detail_ver_kota')) {	
						this.hasAccess.verkota = data_rule.rule.find(o => o.name === 'alsin_detail_ver_kota').active
					}
					if (data_rule.rule.find(o => o.name === 'alsin_detail_ver_kec')) {	
						this.hasAccess.verkec = data_rule.rule.find(o => o.name === 'alsin_detail_ver_kec').active
					}
				}else{
					this.hasAccess = {
						add: true,
        ...
        verifikasi(i, tingkat, val){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/opsindetail/verifikasi/" + data.id,
                data: saveData
            })
                .then((response) => {
        ...
        processVrf(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/opsindetail/vrf_all",
                data: params
            })
            .then((response) => {
