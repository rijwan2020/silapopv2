    ...
    <div>
        <div class="d-flex justify-content-between align-items-center w-100 mb-0 border-bottom">
            ...
            <div class="m-1">
                ...
                <b-btn
					variant="primary"
					v-b-tooltip.hover
					title="Tambah harga tingkat petani"
					@click="showModal('add')"
                    v-if="hasAccess.add"
                    size="sm" >
		...
		<div class="row pt-3 text-center">
			<div class="col-md-12">
				<b-card class="mb-4">
					...
					<h4 class="card-title">{{ data.rata_rata }} Rp/Kg</h4>
				</b-card>
			</div>
		</div>
				...
				>
					...
					<template v-slot:cell(actions)="data">
						<b-btn
							...
							v-b-tooltip.hover
							:title="`Edit Harga tingkat petani`"
						...
						<b-btn
							...
							v-b-tooltip.hover
							:title="`Hapus Harga tingkat petani`"
		...
		<b-modal
			...
			<div slot="modal-title">
				{{ mode == "add" ? "Add" : "Update" }} Harga Tingkat Petani 
			</div>
			...
			<b-form-row>
				<b-form-group label="Harga Tingkat Petani" class="col">
                    <b-input-group>
                        <b-input
                            placeholder="Harga Tingkat Petani"
                            v-model="saveData.htp"
                        />
                        <b-input-group-text slot="append">Rp/Kg</b-input-group-text>
                    </b-input-group>
				</b-form-group>
			</b-form-row>
...
export default {
    ...
    data: () => ({
		...
		fields: [
			...
			{ key: "tanggal", label:"Tanggal", sortable: false, tdClass: "align-middle", thClass: "text-nowrap align-middle text-center" },
			...
			{ key: "htp", label:"Harga Tingkat Petani [Rp/Kg]", sortable: false, tdClass: "align-middle", thClass: "text-nowrap align-middle text-center" },
    ...
    methods:{
        ...
        download(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/htp/download",
                data: params
            }).catch((error) => {
        ...
        update(){
            ...
            axios({
                method: "put",
                url: this.publicUrl + "api/htp/" + this.id,
                data: this.saveData
            })
                .then((response) => {
                    if(this.fileAttachment != null){
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
					...
					if (data_rule.rule.find(o => o.name === 'htp_add')) {	
						this.hasAccess.add = data_rule.rule.find(o => o.name === 'htp_add').active
					}
					if (data_rule.rule.find(o => o.name === 'htp_edit')) {	
						this.hasAccess.edit = data_rule.rule.find(o => o.name === 'htp_edit').active
					}
					if (data_rule.rule.find(o => o.name === 'htp_delete')) {	
						this.hasAccess.delete = data_rule.rule.find(o => o.name === 'htp_delete').active
					}
					if (data_rule.rule.find(o => o.name === 'htp_ver_kota')) {	
						this.hasAccess.verkota = data_rule.rule.find(o => o.name === 'htp_ver_kota').active
					}
					if (data_rule.rule.find(o => o.name === 'htp_ver_kec')) {	
						this.hasAccess.verkec = data_rule.rule.find(o => o.name === 'htp_ver_kec').active
					}
				}else{
					this.hasAccess = {
						add: true,
        ...
        verifikasi(i, tingkat, val){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/htp/verifikasi/" + data.id,
                data: saveData
            })
                .then((response) => {
        ...
        processVrf(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/htp/vrf_all",
                data: params
            })
            .then((response) => {
