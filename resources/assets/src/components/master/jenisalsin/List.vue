    ...
    <div>
		<div class="d-flex justify-content-between align-items-center w-100 mb-0 border-bottom">
			...
			<div class="m-1">
                <b-btn
					variant="primary"
					v-b-tooltip.hover
					title="Tambah Jenis Alsin"
					@click="showModal('add')"
					v-if="hasAccess.jenis_alsin_add"
					size="sm"
				>
				...
				>
					...
					<template v-slot:cell(actions)="data">
						<b-btn
							...
							v-b-tooltip.hover
							:title="`Edit Jenis Alsin`"
							@click="showModal('edit', data.index)"
							v-if="hasAccess.jenis_alsin_edit"
						>
						...
						<b-btn
							...
							v-b-tooltip.hover
							:title="`Hapus Jenis Alsin`"
							@click="deleteModal(data.index)"
							v-if="hasAccess.jenis_alsin_delete"
						>
		...
		>
			<div slot="modal-title">
				{{ mode == "add" ? "Tambah" : "Edit" }} Jenis Alsin
			</div>

            
            <!-- Nama Jenis Alsin -->
			<b-form-row>
				<b-form-group label="Nama Jenis Alsin" class="col">
					<b-input placeholder="Nama Jenis Alsin" v-model="saveData.nama" />
				</b-form-group>
			</b-form-row>
...
export default {
    ...
    methods:{
        ...
        save() {
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/jenisalsin",
                data: this.saveData
            })
                .then((response) => {
        ...
        update(){
            ...
            axios({
                method: "put",
                url: this.publicUrl + "api/jenisalsin/" + this.id,
                data: this.saveData
            })
                .then((response) => {
        ...
        validate() {
            if (this.saveData.nama == "") {
                this.notif("danger", "Error", "Nama Jenis tidak boleh kosong.");
                return false;
            }
            return true;
		},
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
					this.hasAccess = {
						jenis_alsin_add: false,
						jenis_alsin_edit: false,
						jenis_alsin_delete: false,

					}
					let data_rule = this.listLevel.find(o => o.level === this.user.level)
					
					if (data_rule.rule.find(o => o.name === 'jenis_alsin_add')) {	
						this.hasAccess.jenis_alsin_add = data_rule.rule.find(o => o.name === 'jenis_alsin_add').active
					}
					if (data_rule.rule.find(o => o.name === 'jenis_alsin_edit')) {	
						this.hasAccess.jenis_alsin_edit = data_rule.rule.find(o => o.name === 'jenis_alsin_edit').active
					}
					if (data_rule.rule.find(o => o.name === 'jenis_alsin_delete')) {	
						this.hasAccess.jenis_alsin_delete = data_rule.rule.find(o => o.name === 'jenis_alsin_delete').active
					}
				}else{
					this.hasAccess = {
						jenis_alsin_add: true,
						jenis_alsin_edit: true,
						jenis_alsin_delete: true,
					}
				}
			}
		},
