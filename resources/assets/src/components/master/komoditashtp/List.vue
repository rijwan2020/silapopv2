    ...
    <div>
		<div class="d-flex justify-content-between align-items-center w-100 mb-0 border-bottom">
			...
			<div class="m-1">
                <b-btn
					...
					v-if="hasAccess.komoditas_htp_add"
					@click="showModal('add')"
					size="sm"
				>
		...
		<b-card no-body>
			...
			<div class="table-responsive">
				...
				>
                    ...
                    <!-- Kategori Template -->
                    <template v-slot:cell(kategori_id)="data">
                        <b-badge variant="outline-info" v-if="data.item.kategori_id==1">
                            Gabah / Beras
                        </b-badge>
					...
					<template v-slot:cell(actions)="data">
						<b-btn
							...
							v-if="hasAccess.komoditas_htp_edit"
						>
						...
						<b-btn
							...
							v-if="hasAccess.komoditas_htp_delete"
		...
		>
			<div slot="modal-title">
				{{ mode == "add" ? "Tambah" : "Edit" }} Harga Komoditas
			</div>
            ...
            <!-- Kategori -->
            <b-form-row>
                <b-form-group label="Kategori" class="col">
                    <b-select size="md" v-model="saveData.kategori_id">
...
export default {
    ...
    methods:{
        ...
        save() {
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/komoditashtp",
                data: this.saveData
            })
                .then((response) => {
        ...
        update(){
            ...
            axios({
                method: "put",
                url: this.publicUrl + "api/komoditashtp/" + this.id,
                data: this.saveData
            })
                .then((response) => {
        ...
        validate() {
            ...
            if (this.saveData.kategori_id == "Pilih") {
                this.notif("danger", "Error", "Kategori belum dipilih.");
                return false;
            }
            return true;
		},
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
					this.hasAccess = {
						komoditas_htp_add: false,
						komoditas_htp_edit: false,
						komoditas_htp_delete: false,

					}
					let data_rule = this.listLevel.find(o => o.level === this.user.level)
					
					if (data_rule.rule.find(o => o.name === 'komoditas_htp_add')) {	
						this.hasAccess.komoditas_htp_add = data_rule.rule.find(o => o.name === 'komoditas_htp_add').active
					}
					if (data_rule.rule.find(o => o.name === 'komoditas_htp_edit')) {	
						this.hasAccess.komoditas_htp_edit = data_rule.rule.find(o => o.name === 'komoditas_htp_edit').active
					}
					if (data_rule.rule.find(o => o.name === 'komoditas_htp_delete')) {	
						this.hasAccess.komoditas_htp_delete = data_rule.rule.find(o => o.name === 'komoditas_htp_delete').active
					}
				}else{
					this.hasAccess = {
						komoditas_htp_add: true,
						komoditas_htp_edit: true,
						komoditas_htp_delete: true,
					}
				}
			}
		},
