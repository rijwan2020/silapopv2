<template>  
    <div>
			...
			<div class="table-responsive">
				...
				>
					...
					<template v-slot:cell(actions)="data">
						<router-link 
                            :to="{ name: 'desa', params: { kecId: kota.id, id: data.item.id }}"
						...
						<b-btn
							...
							v-if="hasAccess.kecamatan_delete"
						>
		...
		>
            ...
            <!-- Nama Kecamatan -->
			<b-form-row>
				<b-form-group label="Nama Kecamatan" class="col">
					<b-input placeholder="Nama Kecamatan" v-model="saveData.name" />
				</b-form-group>
			</b-form-row>
        ...
        loadData(){
			axios
				.get(this.publicUrl + "api/kecamatan",{
                    params: {kota_id: this.kota.kota_id}
                })
				.then((response) => {
					// handle success
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
					this.hasAccess = {
						kecamatan_add: false,
						kecamatan_edit: false,
						kecamatan_delete: false,
						desa: false,

					}
					let data_rule = this.listLevel.find(o => o.level === this.user.level)
					
					if (data_rule.rule.find(o => o.name === 'kecamatan_add')) {	
						this.hasAccess.kecamatan_add = data_rule.rule.find(o => o.name === 'kecamatan_add').active
					}
					if (data_rule.rule.find(o => o.name === 'kecamatan_edit')) {	
						this.hasAccess.kecamatan_edit = data_rule.rule.find(o => o.name === 'kecamatan_edit').active
					}
					if (data_rule.rule.find(o => o.name === 'kecamatan_delete')) {	
						this.hasAccess.kecamatan_delete = data_rule.rule.find(o => o.name === 'kecamatan_delete').active
					}
					if (data_rule.rule.find(o => o.name === 'desa')) {	
						this.hasAccess.desa = data_rule.rule.find(o => o.name === 'desa').active
					}
				}else{
					this.hasAccess = {
						kecamatan_add: true,
						kecamatan_edit: true,
						kecamatan_delete: true,
						desa: true,
					}
				}
			}
