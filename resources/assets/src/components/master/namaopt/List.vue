    ...
    <div>
		...
		>
			<div slot="modal-title">
				{{ mode == "add" ? "Tambah" : "Edit" }} Nama Opt
			</div>

            
            <!-- Nama OPT -->
			<b-form-row>
				<b-form-group label="Nama OPT" class="col">
					<b-input placeholder="Nama OPT" v-model="saveData.nama" />
				</b-form-group>
			</b-form-row>
...
export default {
    ...
    methods:{
        loadData(){
			...
			axios
				.get(this.publicUrl + "api/namaopt", {
					params: params
				})
				.then((response) => {
					// handle success
        ...
        save() {
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/namaopt",
                data: this.saveData
            })
                .then((response) => {
        ...
        update(){
            ...
            axios({
                method: "put",
                url: this.publicUrl + "api/namaopt/" + this.id,
                data: this.saveData
            })
                .then((response) => {
        ...
        validate() {
            if (this.saveData.nama == "") {
                this.notif("danger", "Error", "Nama Opt tidak boleh kosong.");
                return false;
            }
            return true;
		},
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
					...
					if (data_rule.rule.find(o => o.name === 'nama_opt_add')) {	
						this.hasAccess.add = data_rule.rule.find(o => o.name === 'nama_opt_add').active
					}
					if (data_rule.rule.find(o => o.name === 'nama_opt_edit')) {	
						this.hasAccess.edit = data_rule.rule.find(o => o.name === 'nama_opt_edit').active
					}
					if (data_rule.rule.find(o => o.name === 'nama_opt_delete')) {	
						this.hasAccess.delete = data_rule.rule.find(o => o.name === 'nama_opt_delete').active
					}
				}else{
					this.hasAccess = {
						add: true,
