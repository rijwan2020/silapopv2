    ...
    <div>
		...
		<b-card no-body>
            ...
            <div class="text-center">
                <h4 class="mt-3 mb-1"><b>Laporan Optimalisasi Alsin Tahun {{ tahun }}</b></h4>
                    ...
                    <table class="table table-striped table-bordered">
                        <thead class="text-center">
                            <tr class="text-center">
                                ...
                                <th class="align-middle">Jumlah Alsin [Unit]</th>
                                <th class="align-middle">LTT / LTP [Ha]</th>
                                <th class="align-middle">Realisasi Alsin [Unit]</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(value, i) in data.data" :key="i">
                                <td>{{ i + 1 }}</td>
                                <td>{{ value.nama }}</td>
                                <td class="text-right">{{ value.opsin }}</td>
                        ...
                        <tfoot>
                            <tr class="text-right">
                                ...
                                <th>{{ data.jumlah.opsin }}</th>
                                <th>{{ data.jumlah.ltt_ltp }}</th>
                                <th>{{ data.jumlah.alsin }}</th>
                            </tr>
                        </tfoot>
                    </table>
                </template>
			...
			<div class="row">
                ...
                <div class="col-md-3 px-0 text-right">
                    <b-btn
                        ...
                        @click="download()"
                        class="mt-3"
                        v-if="hasAccess.laporan_opsin_download"
                    >
...
<script>
export default {
	name: 'laporan-optimalisasi-alsin',
	metaInfo: {
		title: 'Laporan Optimalisasi Alsin'
    },
    data: () => ({
		loadParams: {},
		data: [],
    ...
    methods:{
        loadData(){
			...
			axios
				.get(this.publicUrl + "api/laporan/opsin", {
					params: param
				})
				.then((response) => {
					// handle success
        ...
        download(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/laporan/opsin/download",
                data: param
            }).catch((error) => {
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
                    ...
                    this.hasAccess = {
						laporan_opsin_download: false,
						laporan_opsin_print: false,
                    }
					let data_rule = this.listLevel.find(o => o.level === this.user.level)
					if (data_rule) {
						if (data_rule.rule.find(o => o.name === 'laporan_opsin_download')) {	
							this.hasAccess.laporan_opsin_download = data_rule.rule.find(o => o.name === 'laporan_opsin_download').active
						}
						if (data_rule.rule.find(o => o.name === 'laporan_opsin_print')) {	
							this.hasAccess.laporan_opsin_print = data_rule.rule.find(o => o.name === 'laporan_opsin_print').active
						}
					}
				}else{
					this.hasAccess = {
						laporan_opsin_download: true,
						laporan_opsin_print: true,
					}
				}
			}
        },
