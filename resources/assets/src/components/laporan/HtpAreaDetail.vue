		...
		<div class="d-flex justify-content-between align-items-center w-100 mb-0 border-bottom">
            <div class="d-flex align-items-center">
                <h5 class="m-1 d-inline-block text-nowrap font-weight-normal">
                    Laporan / Harga Tingkat Petani / Area / Detail
                </h5>
            </div>
        </div>
			...
			<div class="table-responsive">
                ...
                <template v-else>
                    <table class="table table-striped table-bordered">
                        ...
                        <tfoot>
                            <tr class="text-right">
                                <th colspan="2">Rata Rata [Ha]</th>
                                <th v-for="(avr, j) in data.average" :key="j">
                                    {{ avr.rata_rata }}
                                </th>
                            </tr>
                            <tr class="text-right">
                                <th colspan="2">Min [Ha]</th>
                                <th v-for="(avr, j) in data.average" :key="j">
                                    {{ avr.min }}
                                </th>
                            </tr>
                            <tr class="text-right">
                                <th colspan="2">Max [Ha]</th>
                                <th v-for="(avr, j) in data.average" :key="j">
                                    {{ avr.max }}
                                </th>
                            </tr>
...
export default {
    ...
    methods:{
        loadData(){
			...
			axios
				.get(this.publicUrl + "api/laporan/htp/area/detail", {
					params: params
				})
				.then((response) => {
					// handle success
        ...
        download(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/laporan/htp/area/detail/download",
                data: param
            }).catch((error) => {
        ...
        loadParam(){
            ...
            // Set Komoditi
            if (this.$route.params.komoditi_id) {
                this.pilihKomoditi = this.$route.params.komoditi_id
            }
            // Set Kota
