    ...
    <div>
		...
		<b-card no-body>
            <div class="text-center">
                <h4 class="mt-3 mb-1"><b>Laporan Luas Tanam {{ listBulan[bulan] }} {{ tahun }}</b></h4>
			...
			<div class="row">
                ...
                <div class="col-md-3 px-0 text-right">
                    <b-btn
                        ...
                        @click="download()"
                        class="mt-3"
                        v-if="hasAccess.laporan_luas_tanam_download"
                    >
...
<script>
export default {
	name: 'laporan-luas-tanam-detail',
	metaInfo: {
		title: 'Laporan Luas Tanam'
    },
    data: () => ({
		data: [],
		isBusy: false,
    ...
    methods:{
        loadData(){
			...
			axios
				.get(this.publicUrl + "api/laporan/luas_tanam/detail", {
					params: params
				})
				.then((response) => {
					// handle success
        ...
        download(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/laporan/luas_tanam/detail/download",
                data: param
            }).catch((error) => {
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
                    ...
                    this.hasAccess = {
						laporan_luas_tanam_download: false,
						laporan_luas_tanam_print: false,
                    }
					let data_rule = this.listLevel.find(o => o.level === this.user.level)
					if (data_rule) {
						if (data_rule.rule.find(o => o.name === 'laporan_luas_tanam_download')) {	
							this.hasAccess.laporan_luas_tanam_download = data_rule.rule.find(o => o.name === 'laporan_luas_tanam_download').active
						}
						if (data_rule.rule.find(o => o.name === 'laporan_luas_tanam_print')) {	
							this.hasAccess.laporan_luas_tanam_print = data_rule.rule.find(o => o.name === 'laporan_luas_tanam_print').active
						}
					}
				}else{
					this.hasAccess = {
						laporan_luas_tanam_download: true,
						laporan_luas_tanam_print: true,
					}
				}
			}
        },
