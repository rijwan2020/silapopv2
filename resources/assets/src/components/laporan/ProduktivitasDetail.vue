    ...
    <div>
		...
		<b-card no-body>
            <div class="text-center">
                <h4 class="mt-3 mb-1"><b>Laporan Produktivitas {{ listBulan[bulan] }} {{ tahun }}</b></h4>
                    ...
                    <table class="table table-striped table-bordered">
                        <thead class="text-center">
                            <tr>
                                ...
                                <th :colspan="jml_hari">Tanggal [Kw/Ha]</th>
                                <th class="align-middle" rowspan="2">Jumlah [Kw/Ha]</th>
                            </tr>
                            <tr>
			...
			<div class="row">
                ...
                <div class="col-md-3 px-0 text-right">
                    <b-btn
                        ...
                        @click="download()"
                        class="mt-3"
                        v-if="hasAccess.laporan_produktivitas_download"
                    >
...
<script>
export default {
	name: 'laporan-produktivitas-detail',
	metaInfo: {
    ...
    methods:{
        loadData(){
			...
			axios
				.get(this.publicUrl + "api/laporan/produktivitas/detail", {
					params: params
				})
				.then((response) => {
					// handle success
        ...
        download(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/laporan/produktivitas/detail/download",
                data: param
            }).catch((error) => {
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
                    ...
                    this.hasAccess = {
						laporan_produktivitas_download: false,
						laporan_produktivitas_print: false,
                    }
					let data_rule = this.listLevel.find(o => o.level === this.user.level)
					if (data_rule) {
						if (data_rule.rule.find(o => o.name === 'laporan_produktivitas_download')) {	
							this.hasAccess.laporan_produktivitas_download = data_rule.rule.find(o => o.name === 'laporan_produktivitas_download').active
						}
						if (data_rule.rule.find(o => o.name === 'laporan_produktivitas_print')) {	
							this.hasAccess.laporan_produktivitas_print = data_rule.rule.find(o => o.name === 'laporan_produktivitas_print').active
						}
					}
				}else{
					this.hasAccess = {
						laporan_produktivitas_download: true,
						laporan_produktivitas_print: true,
					}
				}
			}
        },
