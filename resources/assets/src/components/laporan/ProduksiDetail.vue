    ...
    <div>
		...
		<b-card no-body>
            <div class="text-center">
                <h4 class="mt-3 mb-1"><b>Laporan Produksi {{ listBulan[bulan] }} {{ tahun }}</b></h4>
                    ...
                    <table class="table table-striped table-bordered">
                        <thead class="text-center">
                            <tr>
                                ...
                                <th :colspan="jml_hari">Tanggal [Ton]</th>
                                <th class="align-middle" rowspan="2">Jumlah [Ton]</th>
                            </tr>
                            <tr>
			...
			<div class="row">
                ...
                <div class="col-md-3 px-0 text-right">
                    <b-btn
                        ...
                        @click="download()"
                        class="mt-3"
                        v-if="hasAccess.laporan_produksi_download"
                    >
...
export default {
    ...
    methods:{
        loadData(){
			...
			axios
				.get(this.publicUrl + "api/laporan/produksi/detail", {
					params: params
				})
				.then((response) => {
					// handle success
        ...
        download(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/laporan/produksi/detail/download",
                data: param
            }).catch((error) => {
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
                    ...
                    this.hasAccess = {
						laporan_produksi_download: false,
						laporan_produksi_print: false,
                    }
					let data_rule = this.listLevel.find(o => o.level === this.user.level)
					if (data_rule) {
						if (data_rule.rule.find(o => o.name === 'laporan_produksi_download')) {	
							this.hasAccess.laporan_produksi_download = data_rule.rule.find(o => o.name === 'laporan_produksi_download').active
						}
						if (data_rule.rule.find(o => o.name === 'laporan_produksi_print')) {	
							this.hasAccess.laporan_produksi_print = data_rule.rule.find(o => o.name === 'laporan_produksi_print').active
						}
					}
				}else{
					this.hasAccess = {
						laporan_produksi_download: true,
						laporan_produksi_print: true,
					}
				}
			}
        },
