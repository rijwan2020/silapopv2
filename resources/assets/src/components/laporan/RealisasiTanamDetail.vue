    ...
    <div>
		<div class="d-flex justify-content-between align-items-center w-100 mb-0 border-bottom">
            <div class="d-flex align-items-center">
                <h5 class="m-1 d-inline-block text-nowrap font-weight-normal">
                    Laporan / Realisasi Tanam / Detail
                </h5>
            </div>
        </div>
		...
		<b-card no-body>
            <div class="text-center">
                <h4 class="mt-3 mb-1"><b>Laporan Realisasi Tanam {{ listBulan[bulan] }} {{ tahun }}</b></h4>
                    ...
                    <table class="table table-striped table-bordered">
                        <thead class="text-center">
                            <tr>
                                ...
                                <th :colspan="jml_hari">Tanggal [Ha]</th>
			...
			<div class="row">
                ...
                <div class="col-md-3 px-0 text-right">
                    <b-btn
                        ...
                        @click="download()"
                        class="mt-3"
                        v-if="hasAccess.laporan_realisasi_tanam_download"
                    >
...
<script>
export default {
	name: 'laporan-realisasi-tanam-detail',
	metaInfo: {
		title: 'Laporan Realisasi Tanam'
    },
    data: () => ({
		data: [],
		isBusy: false,
    ...
    methods:{
        loadData(){
			...
			axios
				.get(this.publicUrl + "api/laporan/realisasi_tanam/detail", {
					params: params
				})
				.then((response) => {
					// handle success
        ...
        download(){
            ...
            axios({
                method: "post",
                url: this.publicUrl + "api/laporan/realisasi_tanam/detail/download",
                data: param
            }).catch((error) => {
		...
		loadAcl(){
			if (this.user) {
				if (this.user.level != 0) {
                    ...
                    this.hasAccess = {
						laporan_realisasi_tanam_download: false,
						laporan_realisasi_tanam_print: false,
                    }
					let data_rule = this.listLevel.find(o => o.level === this.user.level)
					if (data_rule) {
						if (data_rule.rule.find(o => o.name === 'laporan_realisasi_tanam_download')) {	
							this.hasAccess.laporan_realisasi_tanam_download = data_rule.rule.find(o => o.name === 'laporan_realisasi_tanam_download').active
						}
						if (data_rule.rule.find(o => o.name === 'laporan_realisasi_tanam_print')) {	
							this.hasAccess.laporan_realisasi_tanam_print = data_rule.rule.find(o => o.name === 'laporan_realisasi_tanam_print').active
						}
					}
				}else{
					this.hasAccess = {
						laporan_realisasi_tanam_download: true,
						laporan_realisasi_tanam_print: true,
					}
				}
			}
        },
