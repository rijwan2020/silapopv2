...
class TambahTanamRepository extends BaseRepository
{
    ...
    public function data(Request $request)
    {
        ...
        if ($request->has('kota_id')) {
            ...
            if ($request->has('kecamatan_id')) {
                ...
                $data = $this->sumByKelurahan($filter, "tambah_tanam", "id_group_bulan");
                $res = [];
                ...
                $data = $this->sumByKecamatan($filter, "tambah_tanam", "id_group_bulan");
                $res = [];
                foreach ($area as $value) {
            ...
            $data = $this->sumByKota($filter, "tambah_tanam", "id_group_bulan");
            $res = [];
            foreach ($area as $value) {
                if($value->kota_id != 0){
                    $jumlah = 0;
    ...
    public function download(Request $request)
    {
        ...
        $download['judul_1'] = 'Laporan Luas Tambah Tanam Tahun ' . $request->tahun;
        ...
        dispatch(new BaseJob($download, $createOne->id, 'tambah_tanam'));
        return true;
    }
    ...
    public function detailData(Request $request)
    {
        ...
        if ($request->has('kota_id')) {
            ...
            if ($request->has('kecamatan_id')) {
                $filter['kecamatan_id'] = $request->kecamatan_id;
                $data = $this->sumByKelurahan($filter, "tambah_tanam");
                $area = $this->area->listDesa($filter['kota_id'], $filter['kecamatan_id']);
                $res = [];
                ...
                $area = $this->area->listKecamatan($filter['kota_id']);
                $res = [];
                $data = $this->sumByKecamatan($filter, "tambah_tanam");
                foreach ($area as $value) {
                    if($value->kecamatan_id != 0){
                        $jumlah = 0;
                        $hari = [];
            ...
            $area = $this->area->listKota();
            $res = [];
            $data = $this->sumByKota($filter, "tambah_tanam");            
            foreach ($area as $value) {
                if($value->kota_id != 0){
                    $jumlah = 0;
                    $hari = [];
    ...
    public function detailJumlah(Request $request)
    {
        ...
        if ($request->has('kota_id')) {
            ...
            $dataLuas = $this->sumByKota($filter, "tambah_tanam");
            for ($i=1; $i <= $jml_hari; $i++) {
                $jumlah = 0;
                foreach ($area as $value) {
                    $cari = $value->kota_id . "-". $filter['tahun'] . "-" . sprintf("%02d", $filter['bulan']) . "-" . sprintf("%02d", $i);
    ...
    public function detailDownload(Request $request)
    {
        ...
        $download['judul_1'] = 'Laporan Luas Tambah Tanam ' . $bulan[$request->bulan] . ' ' .$request->tahun;
        ...
        dispatch(new BaseDetailJob($download, $createOne->id, 'tambah_tanam'));
    }
}