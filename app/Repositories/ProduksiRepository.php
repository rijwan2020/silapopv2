...
class ProduksiRepository extends BaseRepository
{
    ...
    public function data(Request $request)
    {
        ...
        if ($request->has('kota_id')) {
            ...
            if ($request->has('kecamatan_id')) {
                ...
                $data = $this->sumByKelurahan($filter, 'produksi', "id_group_bulan");
                $res = [];
                ...
                $data = $this->sumByKecamatan($filter, 'produksi', "id_group_bulan");
                $res = [];
                foreach ($area as $value) {
            ...
            $data = $this->sumByKota($filter, 'produksi', "id_group_bulan");
            $res = [];
            foreach ($area as $value) {
                if($value->kota_id != 0){
                    $jumlah = 0;
    ...
    public function download(Request $request)
    {
        ...
        $download['judul_1'] = 'Laporan Produksi Tahun ' . $request->tahun;
        ...
        $download['judul_4'] = ['#', 'Kota/Kabupaten', 'Bulan [Ton]', '', '', '', '', '', '', '', '', '', '', '', 'Jumlah [Ton]'];
        $download['judul_5'] = [
            '',
            '',
            'Januari',
        ...
        dispatch(new BaseJob($download, $createOne->id, 'produksi'));
        return true;
    }
    ...
    public function detailData(Request $request)
    {
        ...
        if ($request->has('kota_id')) {
            ...
            if ($request->has('kecamatan_id')) {
                $filter['kecamatan_id'] = $request->kecamatan_id;
                $data = $this->sumByKelurahan($filter, 'produksi');
                $area = $this->area->listDesa($filter['kota_id'], $filter['kecamatan_id']);
                $res = [];
                ...
                $area = $this->area->listKecamatan($filter['kota_id']);
                $res = [];
                $data = $this->sumByKecamatan($filter, 'produksi');
                foreach ($area as $value) {
                    if($value->kecamatan_id != 0){
                        $jumlah = 0;
                        $hari = [];
            ...
            $area = $this->area->listKota();
            $res = [];
            $data = $this->sumByKota($filter, 'produksi');            
            foreach ($area as $value) {
                if($value->kota_id != 0){
                    $jumlah = 0;
                    $hari = [];
    ...
    public function detailJumlah(Request $request)
    {
        ...
        if ($request->has('kota_id')) {
            ...
            $dataLuas = $this->sumByKota($filter, 'produksi');
            for ($i=1; $i <= $jml_hari; $i++) {
                $jumlah = 0;
                foreach ($area as $value) {
                    $cari = $value->kota_id . "-". $filter['tahun'] . "-" . sprintf("%02d", $filter['bulan']) . "-" . sprintf("%02d", $i);
    ...
    public function detailDownload(Request $request)
    {
        ...
        $download['judul_1'] = 'Laporan Produksi ' . $bulan[$request->bulan] . ' ' .$request->tahun;
        ...
        $download['judul_4'] = ['#', 'Kota/Kabupaten', 'Tanggal [Ton]'];
        $download['judul_5'] = ['Jumlah [Ton]', ''];
        for ($i=1; $i <= $download['jumlah_hari']; $i++) { 
            $download['judul_4'][] = '';
            $download['judul_5'][] = $i;
        }
        $download['judul_4'][$i + 1] = 'Jumlah [Ton]';
        ...
        dispatch(new BaseDetailJob($download, $createOne->id, 'produksi'));
    }
}