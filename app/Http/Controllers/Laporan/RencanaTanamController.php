...
use App\Jobs\Laporan\RencanaTanamDetailJob;
use App\Jobs\Laporan\RencanaTanamJob;
...
class RencanaTanamController extends BaseController
{
    ...
    public function download(Request $request)
    {
        ...
        $createOne = $this->download->create([
            'keterangan' => 'Laporan Rencana Tanam Tahun ' . $download['tahun'],
            'user_id' => $request->user_id ?? 0
        ]);

        dispatch(new RencanaTanamJob($download, $createOne->id));
        return $this->done();
    }
            ...
            if ($request->has('kecamatan_id')) {
                ...
                foreach ($data as $key => $value) {
                    if($value->kelurahan_id != 0){
                        ...
                        for ($i=1; $i <= 12; $i++) { 
                            $filter['bulan'] = $i;
                            $bulan[] = $this->bakulahan_detail->sumRencanaTanam($filter);
                        }
        ...
        if ($request->has('kota_id')) {
            ...
            if($request->has('kecamatan_id')){
                ...
                for ($i=1; $i <= 12; $i++) { 
                    ...
                    foreach ($data as $key => $value) {
                        $filter['kelurahan_id'] = $value->kelurahan_id;
                        $res[$i] += $this->bakulahan_detail->sumRencanaTanam($filter);
                    }
                }
            }else{
                $data = $this->repo->listKecamatan($filter['kota_id']);
                ...
                foreach ($data as $key => $value) {
                    if($value->kelurahan_id != 0){
                        ...
                        for ($i=1; $i <= $jml_hari; $i++) { 
                            $filter['hari'] = $i;
                            $hari[] = $this->bakulahan_detail->sumRencanaTanam($filter);
                        }
    ...
    protected function detailDownload(Request $request)
    {
        ...
        $createOne = $this->download->create([
            'keterangan' => 'Laporan Rencana Tanam ' . $download['judul'],
            'user_id' => $request->user_id ?? 0
        ]);

        dispatch(new RencanaTanamDetailJob($download, $createOne->id));
        return $this->done();
    }
}
